...
function heuristicClassify(message, spamAnalysis) {
  ...
  if (/gia|bao nhieu|price|bn\b/.test(normalized)) {
    return {
      intent: "ask_price",
      sentiment: "neutral",
      confidence: 0.85,
  ...
  if (/khong nhan|không nhận|te qua|tệ quá|that vong|thất vọng|loi|lỗi/.test(normalized)) {
    return {
      intent: "complaint",
      sentiment: "negative",
      confidence: 0.84,
  ...
  if (/dep qua|đẹp quá|tot qua|tốt quá|uy tin|uy tín|rat ung|rất ưng|xinh qua|xịn quá/.test(normalized)) {
    return {
      intent: "praise",
      sentiment: "positive",
      confidence: 0.83,
